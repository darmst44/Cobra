...
def run_game_loop(game_window, difficulty, apple_count, background_theme):
    ...
    while game_running:
        ...
        # Wrap around if invincible (teleport to opposite side)
        if invincible:
            if snake_pos[0] < 0:
                snake_pos[0] = FRAME_SIZE_X - GRID_SIZE
            elif snake_pos[0] >= FRAME_SIZE_X:
                snake_pos[0] = 0
            
            if snake_pos[1] < 0:
                snake_pos[1] = FRAME_SIZE_Y - GRID_SIZE
        ...
        # Move the star with completely random direction changes
...
def main():
    ...
    while True:
        ...
        # Keep playing with same settings until user goes back to home
        while True:
